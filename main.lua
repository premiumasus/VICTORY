...
-- loadstring(game:HttpGet("https://pastebin.com/raw/9fzdLpnh"))()
-- ========================================
...
local Config = {
    ...
    -- [ตั้งค่าระบบคีย์ดึงจากเว็บ]
    KeyURL = "https://pastebin.com/raw/fxwL0ebu", -- 🔗 วางลิงก์ Raw (เช่น Pastebin Raw / GitHub Raw) ตรงนี้
    MaxAttempts = 2, -- จำนวนครั้งสูงสุดที่อนุญาตให้ใส่คีย์ผิด
    GetKeyURL = "https://premiumasus.github.io/GETKEY/",
    DiscordURL = "https://discord.gg/vcrNKSm2bm",
    
    -- [ข้อความแจ้งเตือนตอนโดนเตะออกจากเกม]
    KickMessage = "( Invalid key; please contact )\n   Discord:\n .gg/vcrNKSm2bm"
}

-- ========================================
-- COLOR SCHEME (THEME: RED & BLACK)
-- ========================================

local Colors = {
    Background = Color3.fromRGB(15, 12, 12),
    Surface = Color3.fromRGB(22, 18, 18),
    Primary = Color3.fromRGB(45, 30, 30),
    Secondary = Color3.fromRGB(35, 25, 25),
    Border = Color3.fromRGB(60, 25, 25),
    TextPrimary = Color3.fromRGB(255, 220, 220),
    TextSecondary = Color3.fromRGB(160, 120, 120),
    
    -- [ข้อความคีย์ตอนพิมพ์: สีแดง]
    KeyInputText = Color3.fromRGB(255, 60, 60),
    ...
    HoverPrimary = Color3.fromRGB(65, 35, 35),
    HoverDiscord = Color3.fromRGB(50, 60, 160),
    HoverGetKey = Color3.fromRGB(30, 120, 80),
    
    -- [ปรับเปลี่ยนโทนขอบนีออนเป็น แดง-ดำ]
    NeonWhite = Color3.fromRGB(255, 30, 30),
    NeonGlow = Color3.fromRGB(120, 0, 0)
}
...
local State = {
    IsLoading = false,
    FailedAttempts = 0, -- ตัวนับจำนวนครั้งที่ใส่คีย์ผิด
    Particles = {},
    Animations = {},
    IsDestroyed = false,
...
-- ========================================
-- KEY SYSTEM WEB FETCHING LOGIC
-- ========================================

local KeyCache = {
    Data = nil,
    LastFetch = 0,
    Timeout = 60 -- โหลดคีย์ใหม่จากเว็บทุกๆ 60 วินาที
}

local function FetchValidKeys()
    local currentTime = os.time()
    if KeyCache.Data and (currentTime - KeyCache.LastFetch) < KeyCache.Timeout then
    ...
    if success and response then
        ...
        for line in string.gmatch(response, "[^\r\n]+") do
            local cleanKey = string.gsub(line, "%s+", "")
            if #cleanKey > 0 then
                validKeys[cleanKey] = true
            end
        end
        KeyCache.Data = validKeys
        KeyCache.LastFetch = currentTime
        return validKeys
    end

    return KeyCache.Data or {}
end

-- ========================================
-- SCRIPT EXECUTION AFTER KEY VERIFIED
-- ========================================

local function LoadMainScript()
    -- ===================================================
    -- >>> ESP BOX LOGIC WITH TOGGLE <<<
    -- ===================================================
    local ESP_Settings = {
        Enabled = true, -- สวิตช์สำหรับ เปิด (true) / ปิด (false) ESP BOX
    ...
    local function ESP(plr)
        local library = {
            blacktracer = NewLine(ESP_Settings.Tracer_Thickness*2, black),
            tracer = NewLine(ESP_Settings.Tracer_Thickness, ESP_Settings.Tracer_Color),
            black = NewQuad(ESP_Settings.Box_Thickness*2, black),
            box = NewQuad(ESP_Settings.Box_Thickness, ESP_Settings.Box_Color),
                ...
                if plr.Character ~= nil and plr.Character:FindFirstChild("Humanoid") ~= nil and plr.Character:FindFirstChild("HumanoidRootPart") ~= nil and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("Head") ~= nil then
                    ...
                    if OnScreen then
                        ...
                        if ESP_Settings.Tracers then
                            if ESP_Settings.Tracer_Origin == "Middle" then
                                library.tracer.From = camera.ViewportSize*0.5
                                library.blacktracer.From = camera.ViewportSize*0.5
                            elseif ESP_Settings.Tracer_Origin == "Bottom" then
                                library.tracer.From = Vector2.new(camera.ViewportSize.X*0.5, camera.ViewportSize.Y) 
                                library.blacktracer.From = Vector2.new(camera.ViewportSize.X*0.5, camera.ViewportSize.Y)
                            end
                            if ESP_Settings.Tracer_FollowMouse then
                        ...
                        if Team_Check.TeamCheck then
                            ...
                            library.tracer.Color = ESP_Settings.Tracer_Color
                            library.box.Color = ESP_Settings.Box_Color
                        end
    ...
    -- ฟังก์ชันสำหรับใช้ Toggle ESP ผ่านการเรียกใช้ในสคริปต์
    ...
    -- โหลดสคริปต์ภายนอกตามโครงเดิมของคุณ
    pcall(function()
        loadstring(game:HttpGet("https://pastebin.com/raw/e2setW0K"))()
    end)
end
...
local function CreateHeader(parent)
    ...
    iconImage.Position = UDim2.new(0.05, 0, 0.05, 0)
    iconImage.BackgroundTransparency = 1
    iconImage.Image = "rbxassetid://117566351483819"
...
local function CreateInputSection(parent)
    ...
    section.Position = UDim2.new(0, 0, 0, 90)
    ...
    textInput.TextColor3 = Colors.KeyInputText
...
local function CreateButtons(parent)
    ...
    submitButton.Position = UDim2.new(0, 0, 0, 190)
    ...
    buttonsContainer.Position = UDim2.new(0, 0, 0, 250)
...
local function CreateParticle()
    ...
    highlight.BackgroundColor3 = Color3.fromRGB(255, 200, 200)
...
-- ========================================
-- REAL KEY VALIDATION LOGIC
-- ========================================

local function ValidateKey()
    if State.IsLoading or State.IsDestroyed then return end
    
    local rawInputKey = UI.Input.TextBox.Text
    local cleanInputKey = string.gsub(rawInputKey, "%s+", "")
    
    if cleanInputKey == "" then
    ...
    SetLoading(true)
    ShowStatus("Validating key from server...", false, false)
    
    task.spawn(function()
        local validKeys = FetchValidKeys()
        SetLoading(false)
        
        if validKeys[cleanInputKey] then
            ShowStatus("Access Granted! Loading script...", false, true)
            task.wait(1)
            ...
            if State.FailedAttempts >= Config.MaxAttempts then
                ShowStatus("Max attempts reached! Kicking...", true, false)
                task.wait(1)
                
                Player:Kick(Config.KickMessage)
            else
                local remaining = Config.MaxAttempts - State.FailedAttempts
                ShowStatus("Invalid Key! You have " .. remaining .. " attempt left.", true, false)
            end
        end
    end)
end
...
local function ConnectEvents()
    ...
    UI.Buttons.GetKey.MouseButton1Click:Connect(function()
        ...
        CopyToClipboard(Config.GetKeyURL, "Key website link copied to clipboard!")
    end)
